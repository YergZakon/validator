...
def annotate_page(request: Request, article_db_id: int, filter: str = "all"):
    ...
    # Типы связей для модификации (18 — актуальный список после v18 + правок методолога).
    # Группы: семантика (10), процедура (3), структура (3), классификация (2).
    RELATION_TYPES = [
        # Семантические (Layer A)
        "ПРИМЕНЯЕТСЯ_К", "НАПРАВЛЕНО_НА", "АДРЕСОВАНО_СУБЪЕКТУ",
        "УСТАНАВЛИВАЕТ_МОДАЛЬНОСТЬ", "ИМЕЕТ_УСЛОВИЕ", "ИСКЛЮЧАЕТ",
        "ИМЕЕТ_ПАРАМЕТР", "ВЛЕЧЁТ", "ОПРЕДЕЛЯЕТ", "АЛЬТЕРНАТИВА",
        # Процедурные / каузальные / иерархические выводы
        "ОСУЩЕСТВЛЯЕТСЯ_ПО", "ОБУСЛАВЛИВАЕТ", "КОНКРЕТИЗИРУЕТ",
        # Структурные (Layer B, авто)
        "ИЕРАРХИЯ", "СОСТОИТ_ИЗ", "ССЫЛАЕТСЯ_НА", "ДЕЙСТВУЕТ_В",
