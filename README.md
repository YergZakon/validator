...
## Версионирование схемы связей

| Версия | Связей | Что нового |
|---|---|---|
| Phase 1 | 10 типов / 1956 экземпляров | Базовое ядро: SAO, деонтика, defeasibility, параметры |
| **Phase 2** | **11 типов / 2052 экземпляров** | **+ `СООТВЕТСТВУЕТ_ТИПУ`** — замыкает Слой 3 (модальная характеристика) и Слой 4 (тип нормы) |

`rel_id` — стабильный sha1-хеш от `(article_id, from_id, to_id, relation_type)`. Добавление новых типов связей не сдвигает ID существующих, и голоса экспертов в БД переживают `seed.py` благодаря UPSERT-логике (`ON CONFLICT(article_db_id, rel_id) DO UPDATE`).
